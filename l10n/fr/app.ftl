...
retcn-read-current-device = Lire depuis l’appareil actuel
retcn-read-current-device-reading = Lecture des informations de l’appareil actuel…
retcn-read-current-device-complete = Les informations de l’appareil ont été renseignées. Vous pouvez modifier toute valeur avant de rechercher le firmware.
retcn-read-current-device-unsupported = Cette fonctionnalité ne peut pas être utilisée sur l’appareil actuel. Elle est disponible uniquement sur les appareils Lenovo et Motorola.
retcn-read-current-device-permission-denied = L’autorisation nécessaire pour lire l’IMEI a été refusée. Veuillez saisir l’IMEI manuellement.