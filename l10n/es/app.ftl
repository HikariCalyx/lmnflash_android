...
retcn-read-current-device = Leer desde el dispositivo actual
retcn-read-current-device-reading = Leyendo información del dispositivo actual…
retcn-read-current-device-complete = La información del dispositivo se ha completado. Puede editar cualquier valor antes de buscar firmware.
retcn-read-current-device-unsupported = Esta función no se puede usar en el dispositivo actual. Solo está disponible en dispositivos Lenovo y Motorola.
retcn-read-current-device-permission-denied = Se denegó el permiso necesario para leer el IMEI. Introduzca el IMEI manualmente.