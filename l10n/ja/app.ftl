...
retcn-read-current-device = 現在のデバイスから読み取る
retcn-read-current-device-reading = 現在のデバイス情報を読み取り中…
retcn-read-current-device-complete = デバイス情報が入力されました。ファームウェアを検索する前に、任意の値を編集できます。
retcn-read-current-device-unsupported = この機能は現在のデバイスでは使用できません。Lenovo または Motorola のデバイスでのみ利用できます。
retcn-read-current-device-permission-denied = IMEI の読み取りに必要な権限が拒否されました。IMEI を手動で入力してください。