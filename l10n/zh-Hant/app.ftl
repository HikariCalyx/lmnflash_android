...
retcn-read-current-device = 從目前裝置讀取
retcn-read-current-device-reading = 正在讀取目前裝置資訊…
retcn-read-current-device-complete = 已填入裝置資訊。您可以在查詢韌體前編輯任何值。
retcn-read-current-device-unsupported = 此功能無法在目前裝置上使用。它僅適用於 Lenovo 和 Motorola 裝置。
retcn-read-current-device-permission-denied = 讀取 IMEI 所需的權限遭到拒絕。請手動輸入 IMEI。