...
history-button = Sökhistorik
history-title = Sökhistorik
history-empty = Inga lyckade sökningar ännu.
history-clear = Rensa
history-close = Stäng
history-query = Sökning: { $query }
history-recorded = Sparad: { $time }
history-details = Visa detaljer
history-psn = PSN
history-manage = Hantera
history-remove = Ta bort ({ $count })
history-done = Klar
history-select-all = Välj alla
retcn-read-current-device = Läs från aktuell enhet
retcn-read-current-device-reading = Läser information från den aktuella enheten…
retcn-read-current-device-complete = Enhetsinformationen har fyllts i. Du kan redigera valfritt värde innan du söker efter fast programvara.
retcn-read-current-device-unsupported = Den här funktionen kan inte användas på den aktuella enheten. Den är endast tillgänglig på Lenovo- och Motorola-enheter.
retcn-read-current-device-permission-denied = Den behörighet som krävs för att läsa IMEI nekades. Ange IMEI manuellt.