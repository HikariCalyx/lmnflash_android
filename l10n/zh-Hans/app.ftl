...
retcn-read-current-device = 从当前设备读取
retcn-read-current-device-reading = 正在读取当前设备信息…
retcn-read-current-device-complete = 已填充设备信息。您可以在查询固件前编辑任何值。
retcn-read-current-device-unsupported = 此功能无法在当前设备上使用。它仅适用于联想和摩托罗拉设备。
retcn-read-current-device-permission-denied = 读取 IMEI 所需的权限被拒绝。请手动输入 IMEI。