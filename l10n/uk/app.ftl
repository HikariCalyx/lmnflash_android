...
history-button = Історія пошуку
history-title = Історія пошуку
history-empty = Успішних пошуків ще немає.
history-clear = Очистити
history-close = Закрити
history-query = Запит: { $query }
history-recorded = Збережено: { $time }
history-details = Показати деталі
history-psn = PSN
history-manage = Керувати
history-remove = Видалити ({ $count })
history-done = Готово
history-select-all = Вибрати все
retcn-read-current-device = Зчитати з поточного пристрою
retcn-read-current-device-reading = Зчитування інформації з поточного пристрою…
retcn-read-current-device-complete = Дані пристрою заповнено. Перед пошуком мікропрограми можна змінити будь-яке значення.
retcn-read-current-device-unsupported = Цю функцію не можна використовувати на поточному пристрої. Вона доступна лише на пристроях Lenovo та Motorola.
retcn-read-current-device-permission-denied = Необхідний дозвіл на зчитування IMEI відхилено. Введіть IMEI вручну.