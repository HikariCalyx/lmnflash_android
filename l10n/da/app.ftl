...
retcn-read-current-device = Læs fra den aktuelle enhed
retcn-read-current-device-reading = Læser oplysninger om den aktuelle enhed…
retcn-read-current-device-complete = Enhedsoplysninger er udfyldt. Du kan redigere alle værdier, før du søger efter firmware.
retcn-read-current-device-unsupported = Denne funktion kan ikke bruges på den aktuelle enhed. Den er kun tilgængelig på Lenovo- og Motorola-enheder.
retcn-read-current-device-permission-denied = Den nødvendige tilladelse til at læse IMEI blev afvist. Indtast IMEI manuelt.