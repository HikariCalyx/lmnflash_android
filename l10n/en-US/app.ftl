...
retcn-read-current-device = Read From Current Device
retcn-read-current-device-reading = Reading current device information…
retcn-read-current-device-complete = Device information filled. You can edit any value before looking up firmware.
...
retcn-read-current-device-unsupported = This feature cannot be used on the current device. It is only available on Lenovo and Motorola devices.
retcn-read-current-device-permission-denied = Permission required for reading IMEI is denied. Please input IMEI manually.