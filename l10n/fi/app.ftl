...
retcn-read-current-device = Lue nykyisestä laitteesta
retcn-read-current-device-reading = Luetaan nykyisen laitteen tietoja…
retcn-read-current-device-complete = Laitteen tiedot on täytetty. Voit muokata mitä tahansa arvoa ennen laiteohjelmiston hakua.
retcn-read-current-device-unsupported = Tätä ominaisuutta ei voi käyttää nykyisellä laitteella. Se on saatavilla vain Lenovo- ja Motorola-laitteilla.
retcn-read-current-device-permission-denied = IMEI:n lukemiseen tarvittava lupa evättiin. Syötä IMEI manuaalisesti.