...
retcn-read-current-device = Vom aktuellen Gerät lesen
retcn-read-current-device-reading = Informationen vom aktuellen Gerät werden gelesen…
retcn-read-current-device-complete = Die Geräteinformationen wurden ausgefüllt. Sie können jeden Wert vor der Firmware-Suche bearbeiten.
retcn-read-current-device-unsupported = Diese Funktion kann auf dem aktuellen Gerät nicht verwendet werden. Sie ist nur auf Lenovo- und Motorola-Geräten verfügbar.
retcn-read-current-device-permission-denied = Die erforderliche Berechtigung zum Lesen der IMEI wurde verweigert. Bitte geben Sie die IMEI manuell ein.